...
export const calculateHeatLoad = (
  ...
  const doorWidth = miscData.doorClearOpeningWidth || 900; // mm
  const doorHeight = miscData.doorClearOpeningHeight || 2000; // mm
...
export const calculateFreezerHeatLoad = (
  ...
  const productMass = convertMass(productData.massBeforeFreezing, productData.massUnit, 'kg');
  const dailyLoadingKg = convertMass(miscData.dailyLoading, productData.massUnit, 'kg');
  const respirationMassKg = convertMass(productData.respirationMass, productData.massUnit, 'kg');
  ...
  const doorWidth = miscData.doorClearOpeningWidth || 900; // mm
  const doorHeight = miscData.doorClearOpeningHeight || 1800; // mm
  ...
  return {
    ...
    heaterLoad: heaterLoad / 1000,
    steamHumidifierLoad: steamHumidifierLoad / 1000,
    totalMiscLoad: totalMiscLoad / 1000,
    totalLoad: totalLoadKw * 1000, // Convert to Watts
    totalLoadTR,
    capacityTR,
    sensibleHeat: sensibleHeat / 1000,
    latentHeat: latentHeat / 1000,
    airQtyRequired,
    loadInKJ: totalLoadKJ,
    loadInKw,
    loadInBtu,
    refrigerationCapacity: refrigerationCapacityTR * 12000,
    wallTempDiff,
    ceilingTempDiff,
    floorTempDiff,
    productTempDiff,
    ...
    storageCapacityValid: true,
    maxStorageCapacity: internalVolume * 35.28 * (miscData.storageCapacity || 8),
